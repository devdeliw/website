...
in Part 1, we can calculate the energy a second electron at $\vec{r}$ gets from this field. 

A second electron has magnetic moment $\vec{\mu}_2$. So we get 

$$ 
\begin{align}
U = -\vec{\mu}_2 \cdot \vec{B}_1(\vec{r}) = \frac{\mu_0}{4\pi r^3} [ -\mu_1
\cdot \mu_2 + 3 (\mu_1 \cdot \hat{r})(\mu_2 \cdot \hat{r})]. 
\end{align}
$$
...
$$
\hat{\vec{\mu}}_i = -g_e \mu)B \frac{\hat{\vec{S}}_i}{\hbar}, \quad \mu_B =
\frac{e\hbar}{2m_e}. 
$$

Therefore, replacing $\vec{\mu}\_{1, 2}$ with the quantum operator(s)
...
$$
\begin{align} 
\hat{H}_{ZFS} &= -\frac{\mu_0}{4\pi} \frac{(g_e\mu_B)^2}{\hbar^2} \frac{1}{r^2}
...
$$
\begin{align}
\hat{H}\_{ZFS} = \sum\_{i, j \in \{x, y, z\}} S\_{1i}D_{ij}(\vec{r})S\_{2i} =
...
defect's principle axis. We start by simplifying Equation (12). If we take the
trace of the expression in the parenthesis, we find 

$$
\begin{align*}
\sum_i \left( \frac{\delta_{i, i}}{r^3} - \frac{3r_i r_j}{r^5} \right) &= \sum
\frac{1}{r^3} - 3 \sum \frac{r_i^2}{r^5} \\\\ 
...
makes the $D$ symmetric along $x \leftrightarrow y$ directions; not sure. Regardless, we
...
$$
\begin{align} 
D &= \frac{3}{2}D_z \\\\ \qquad E &= \frac{1}{2}(D_x - D_y) 
\end{align}
$$
